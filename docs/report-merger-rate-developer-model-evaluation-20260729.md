...
| **A2** | ✅ **done** | New `skills/lint/` — differential linter, 59 tests, self-lints clean |
...
| **A4** | ❌ **declined — already shipped** | The rule exists in `code-review/SKILL.md`, in a stronger form, and the run artefacts show it working. See §0.6 |
| **A5** | ❌ **declined; replaced** | `run.json` is already the manifest. Replaced by a 4-line `run-report.md` header addition. See §0.6 |
| **A6** | ✅ **done** | One clause in PM `SKILL.md` step 3: read the output a human will read, not just the exit code |
| **A7** | ✅ **done, re-cut** | Adopted as a *cost* test, not a ruling counter — a counter would have stopped baseline-3. See §0.6 |
| **A8** | ✅ **done** | One clause in PM `SKILL.md` step 3: close or explicitly re-carry every inherited note before accepting |
| **A9** | 🔴 **new, blocking** | **`ruff` and `markdownlint-cli2` are not installed on this machine** — A2/A3 currently yield coverage gaps and catch nothing. See §0.6 |
| **P1** | ✅ **done** | Numerical Domain Contract split into form (always in scope, pre-coercion) and magnitude (out of scope) axes |
| P2–P4 | ❌ **declined** | Left in the plan deliberately — they are the test. See §0.6 |
| R1–R4 | ⬜ open | Next runs, unchanged — §0.3. **Do A9 first**, or the headline experiment measures nothing |
...
| **A4** ❌ | *Declined — already shipped in `code-review`, and the run artefacts show it working (§0.6).* **Add the reviewer reachability rule** — verbatim: *"No finding may be rated P0 or P1 unless the report names a concrete caller in this repository that can produce the offending input. Otherwise it is P3 or it is dropped."* | `references/reviewer-prompt.md` | Free, model-independent, and would have eliminated nearly every over-escalation observed. Sol's problem is calibration, not capability | §7.8 |
| **A5** ❌ | *Declined — `run.json` already is the manifest; replaced by a run-report header addition (§0.6).* **Emit a machine-readable run manifest** (`manifest.json`): harness, model, **effort**, plan sha256, reviewer, attempt cap, tool versions — read from the transcript, not operator input. Refuse to start when the declared config disagrees | `pm.py init` | Two of three stated invariants in this round silently did not hold. This is what makes future comparisons trustworthy | §7.1 |
| **A6** ✅ | *Done as a PM judgement clause, not a toolkit capture — the toolkit cannot know a repo's entry command (§0.6).* **Read the output a human will read**, not just the exit code | PM SKILL step 3 | The only artefact that reveals presentation defects. opencode-1's redshift-column bug passed every DoD checkbox | §7.6 |
| **A7** ✅ | *Done, re-cut as a cost test — a ruling counter would have stopped baseline-3 (§0.6).* ~~Count adjudications per plan section within a run; past ~3 rulings on one section, stop~~ → **a section that keeps costing attempts, or two reviews contradicting each other on it, is a plan defect** | PM SKILL judgement guidance | Bounds the escalation loop that killed run 1, without giving the PM plan-editing authority | §7.9 |
| **A8** ✅ | *Done as one clause, not new state — the motivating case is now caught by `ruff F841` (§0.6).* **Close or explicitly re-carry every inherited note item** before accepting | PM SKILL step 3 | `stored_edges` shipped dead despite being named in the PM's own Slice-1 notes | §4.1 |
...
| # | Action | Why |
|---|---|---|
| **P1** ✅ | *Done — split into form/magnitude axes; plan sha256 changed, re-freeze for R1 (§0.6).* **Disambiguate the Numerical Domain Contract.** State that *dtype / rank / shape / sign / finiteness / complex* are **always in scope and validated before any coercion**, `AssertionError` required; only *magnitude* extremes are out of domain. Add: "a guard that rejects in-domain input is a defect of equal severity to a missing guard" | Two reviewers read the current text to **opposite** conclusions in run 3. Direct cause of opencode-1's undetected non-compliance, ≥3 steers in run 5, and opencode-2's over-strict float-only guard |
| **P2** ❌ | *Declined — A6 covers the class generically, and writing it into the plan removes the only live test of whether the system catches it (§0.6).* Add to Slice 4 DoD: *"every printed data row identifies its own redshift and mass bin"* | The one defect class no linter catches — and exactly what sank opencode-1 |
| **P3** ❌ | *Declined — lint is already in the Developer prompt and PM assess (A3), and authorized surface is floor fact 5; restating both confounds attribution (§0.6).* Add a global DoD item: *"no new lint findings; no new warnings; diff touches only the authorized surface"* | Five branches shipped 11 such items between them |
| **P4** ❌ | *Declined, kept as deliberate plan noise — inferable from the code, and baseline-2 got it right by reading (§0.6).* Specify the `AGENTS.md` architecture explicitly | 3 of 4 branches drew the diagram wrong |

*Deliberately **not** doing:* a persistent cross-run plan-defect ledger. Rediscovering plan defects each run is accepted as realistic and is itself worth testing (§7.9). P2–P4 are declined on the same reasoning: **only P1 was fixed, because only P1 is self-contradictory** and therefore corrupts grading rather than testing the system's adaptability.
...
### 0.6 Implementation record — A4–A8, P1–P4 (2026-07-29, second pass)

Independently assessed before implementing. **Two of the five toolkit actions were declined on evidence**, and three of the four plan edits were declined on design grounds. Net change: three sentences in `project-manager/SKILL.md`, four lines in `state.render_run_report`, two small `lint` changes, and one plan section rewritten. No new commands, no new state fields, no ninth floor fact.

**A4 — declined. The rule already exists and is already working.** `code-review/SKILL.md:92` gates `P0`/`P1` on a stated reachability path, and covers more ground than the proposed sentence (CLI invocations, documented public surface, plan-declared input domains). It landed in `ai-agent-coder` commit `bc65650` at **2026-07-28T03:40Z**; runs 2–5 began 07:58Z, 10:29Z, 12:29Z and 23:14Z, all after it. Skills are symlinked from the repo into `~/.claude/skills` and `prompts.compile_skill_bundle` reads the repo at review time, so the live reviewer bundle carried it. It is also demonstrably in force: across run 5's **29 review reports** the severity split is **4 P1 / 16 P2 / 1 P3** for `code-review`, and every P1 names a concrete producer path — e.g. *"`_save_pairs` persists these values directly from `z` and config at `src/calc.py:55,59-60`, so a legal configuration such as `redshifts=[2]` … produces integer HDF5 attributes that this calculation rejects."* That is not an over-escalating reviewer. §4.3's calibration diagnosis draws substantially on run 1, which predates the rule. **The residual reviewer disagreement is plan-caused, not calibration-caused** — see P1 below — so a second copy of the rule in `references/reviewer-prompt.md` would duplicate a contract that already has one home and change nothing.

*Watch item, not yet acted on:* `drift-audit` has no severity ladder at all (its only mention of severity is a `[P1]` in an output example) and returned **8 P1 / 8 P2** against `code-review`'s 4 P1. But the harm it actually caused was raising *legitimate* form validation as unauthorized scope, which P1 below fixes at the source. Re-measure after the next run before touching the skill.

**A5 — declined; replaced by four lines.** `run.json` already records harness, model, effort, plan sha256, reviewer tools/model/effort and `max_attempts`, HMAC-signed and tamper-evident: it *is* the manifest. Its `effort` is the flag `profiles.compose_launch_command` actually composed, which is strictly better evidence than a statusline or a transcript field. Reading configuration back out of per-harness transcripts is version-fragile machinery for a failure this report already attributes elsewhere — *"the discrepancy is in the test log, not the tooling"* (§1.1). What was genuinely missing was visibility: `render_run_report` printed the harness but not the reviewer or the attempt budget, and the reviewer is exactly the control that was violated in run 1a. Both now appear in every run report header, from state already held.

**A6 — done.** One clause in PM `SKILL.md` step 3: when a slice produces output a human will read — a results table, a figure, CLI text, a doc — read that output itself, not just the exit code. Not mechanisable in the toolkit, which cannot know a given repo's entry command. This is also why P2 is declined.

**A7 — done, but re-cut.** A ruling counter would have stopped the best-behaved run in the set: baseline-3 rejected the same finding class ~10 times on cited measurements and scored A−, which §7.9 itself calls the system working. The counter measures the wrong quantity. Adopted instead as a **cost** test, folded into the existing plan-vs-model bullet: free adjudication is unbounded and healthy; one plan section that keeps *costing attempts*, or two commissioned reviews reaching opposite conclusions about it, is evidence of a defective plan and a materially better stop reason than "developer failed". This also absorbs §7.6 #8 (reviewer disagreement as signal) without a separate rule.

**A8 — done, minimal.** The motivating case is now caught mechanically: `stored_edges` is a dead local, i.e. `ruff F841`, in the default rule set — A2/A3 closes it without any new state. The residual is only non-mechanical carry-forwards, which does not justify a `pm carry` command when `notes.md` is already the channel. One clause in step 3: before accepting, every inherited open item is either closed with the evidence that closes it or explicitly re-carried with a reason.

**A9 — new, and it outranks everything above.** `ruff`, `markdownlint-cli2` and `codespell` are **not installed on this machine**. The lint skill fails closed correctly — a missing linter is `unavailable` coverage, never a pass — so nothing is unsafe, but A2/A3 is this report's number-one lever, credited with 11 of ~14 non-numerical findings, and until the binaries exist every run will record a coverage gap and catch none of them. Two changes shipped alongside: `codespell` gained a `uv` recipe so both Python linters resolve to one install path instead of `ruff`→`uv` and `codespell`→`pip install --user` on the same machine; and `check`/`detect` now share a hint that names the missing binaries, states that those languages were **not** checked, prints the install command, and points at `uv` when absent. `check` still never installs, and `SKILL.md` now says outright that an agent finding a tool missing reports it and lets the human decide.

**P1 — done.** This is the one plan defect that had to be fixed, because it is a *contradiction*, not an under-specification: the contract listed "values outside a stated domain" among malformed inputs every function must reject, then declared out-of-domain behaviour unspecified and un-reportable. The run-5 artefacts show the collision directly — `drift-audit` raising *"Adds out-of-domain dtype validation forbidden by the frozen contract"* as P1 while `code-review` raises *"coerces malformed inputs before validating dtype"* as P1, on the same code. No implementation can satisfy both readings, so the defect corrupts *grading* rather than testing adaptability. The section is now split into **Axis 1 — form** (dtype including complex and string, rank, shape, sign, finiteness; always in scope, validated **before any coercion**; "numeric" means real integer or floating, so rejecting an integer is rejecting valid data) and **Axis 2 — magnitude** (unspecified outside the declared ranges, and that exemption never excuses a missing form check), plus the explicit rule that **a guard rejecting in-domain input is a defect of equal severity to a missing guard** and every rejection message must name the actual reason. The Slice-4 non-goal was narrowed to "out-of-domain *magnitudes*" to match. **The plan sha256 has changed; re-freeze and record it for R1.**

**P2 — declined.** A6 covers the class generically. Writing "every printed row identifies its own redshift" into the plan would remove the only live test of whether the system catches a presentation defect on its own, which is precisely the behaviour under study.

**P3 — declined.** Redundant and confounding: lint is already in the Developer prompt and the PM assess step (A3), and authorized surface is floor fact 5, mechanical and non-waivable. Restating both in the plan duplicates enforcement contracts and makes it impossible to attribute compliance to the system rather than to the plan text.

**P4 — declined, deliberately left as an imperfection.** The `AGENTS.md` architecture is under-specified but inferable from the code, and baseline-2 got it right by reading. It discriminates on exactly the care-with-documentation axis under study, and unlike P1 it is not self-contradictory. Good realistic plan noise; keep it.

**Verification.** PM suite **307/307** green (including `TestPaneTextRejoinsHardWraps`, previously the one environment-dependent failure); lint suite **59/59** (56 + 3 new); `py_compile` clean on all changed Python; CI no-baggage greps and doc-link reachability pass locally.
